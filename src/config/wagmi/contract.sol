...
contract Sale is AccessControl, Utils, Initializable {
    ...
    function initialize(
        ...
        require(
            syntrumTokenAddress != address(0),
            "syntrumTokenAddress can not be zero"
        );
        require(
            receiverAddress != address(0),
            "receiverAddress can not be zero"
        );
        require(
            syntrumTokenAddress != address(0),
            "syntrumTokenAddress can not be zero"
    ...
    function _checkSale() internal returns (bool) {
        if (_activeRoundIndex == 0 && _rounds[1].startTime < block.timestamp) {
            _activeRoundIndex = 1;
        }
        ...
        for (uint8 i = activeRoundIndex; i <= _roundsNumber; i++) {
            if (_rounds[i].endTime < block.timestamp) {
                ...
                if (_rounds[i].maxAllocation > _rounds[i].allocated) {
                    _rounds[_activeRoundIndex].maxAllocation += (_rounds[i]
                        .maxAllocation - _rounds[i].allocated);
                }
            } else {
                break;
            }
    ...
    function checkPermission(
        ...
        address signer = keccak256(message).toEthSignedMessageHash().recover(
            signature
        );
    ...
    function getRate(address tokenAddress) public view returns (uint256) {
    ...
    function getSyntrumUsdRate(bool needUpdate) public view returns (uint256) {
    ...
    function getActiveRoundIndex() public view returns (uint8) {
        uint8 activeRoundIndex = _activeRoundIndex;
        if (activeRoundIndex == 0 && _rounds[1].startTime < block.timestamp) {
            activeRoundIndex = 1;
        }
    ...
    function isSaleActive() external view returns (bool) {
        uint8 activeRoundIndex = getActiveRoundIndex();
        return activeRoundIndex > 0 && activeRoundIndex < _maxRoundsNumber;
    }
    ...
    function setPublic(bool isPublic) external hasRole(MANAGER) returns (bool) {
        _isPublic = isPublic;
        return true;
    }
    ...
    function purchase(
        ...
        if (
            _paymentProfiles[paymentProfileIndex].contractAddress != address(0)
        ) {
            require(
                paymentAmount > 0,
            ...
            require(
                msg.value == 0,
                "msg.value should be zero for this payment profile"
            );
        } else {
        ...
        uint256 purchaseAmount = (usdPaymentAmount * SHIFT_18) / syntrumUsdRate;
        if (
            purchaseAmount >
            _rounds[_activeRoundIndex].maxAllocation -
                _rounds[_activeRoundIndex].allocated &&
            ...
            uint256 currentRoundPurchase = _rounds[_activeRoundIndex]
                .maxAllocation - _rounds[_activeRoundIndex].allocated;

            uint256 nextRoundUsdPayment = usdPaymentAmount -
                ((currentRoundPurchase * syntrumUsdRate) / SHIFT_18);

            _rounds[_activeRoundIndex].allocated = _rounds[_activeRoundIndex]
                .maxAllocation;
            _activeRoundIndex++;
            syntrumUsdRate = getSyntrumUsdRate(false);

            _rounds[_activeRoundIndex].allocated =
                (nextRoundUsdPayment * SHIFT_18) /
                syntrumUsdRate;
            purchaseAmount =
                currentRoundPurchase +
                _rounds[_activeRoundIndex].allocated;
        } else {
            require(
                purchaseAmount + _rounds[_activeRoundIndex].allocated <=
                    _rounds[_activeRoundIndex].maxAllocation,
    ...
    function withdrawAvailable() external returns (bool) {
        ...
        require(withdrawAmount > 0, "No tokens available for withdraw");
        return _withdraw(msg.sender, withdrawAmount);
    }
    ...
    function addPaymentProfile(
        ...
        _paymentProfiles[_paymentProfilesNumber]
    ...
    function setPaymentProfileWeight(
        ...
        require(
            paymentProfileIndex > 0 &&
                paymentProfileIndex <= _paymentProfilesNumber,
    ...
    function setRoundsData(
        ...
        for (uint256 i = 0; i < usdRate.length; i++) {
            ...
            _rounds[roundsNumber].endTime =
                _rounds[roundsNumber].startTime +
    ...
    function setRoundUsdRate(
        ...
        require(roundIndex <= _roundsNumber, "Invalid round number");
    ...
    function setRoundTime(
        ...
        if (roundIndex > 1) {
            require(
                startTime >= _rounds[roundIndex - 1].endTime,
                "Round can not start before previous round end"
            );
        }
        if (roundIndex < _roundsNumber) {
            require(
                endTime <= _rounds[roundIndex + 1].startTime,
    ...
    function getUserAvailable(
        ...
        uint256 toWithdraw = ((_purchasedAmount[userAddress] +
            _referralAmount[userAddress]) * getAvailablePercentage()) / SHIFT_4;
